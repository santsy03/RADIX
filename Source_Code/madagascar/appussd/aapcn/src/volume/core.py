...
from aapcn.src.common.core import update_uc_ut, provision_refill
from aapcn.src.common.core import get_da_balance,update_parabole_offer,update_parabole_da
from config import DA_BALANCE
from config import PARABOLE_OFFER,PARABOLE_DA
        ...
        if resp[2] == True:
            try:
                ...
                if resp_2[2] == True:
                    ...
                    #'''
                    #Update Parabole offer and DA with end of month expiry
                    if resources['parameters']['package_name'] == 'MyGig100_Parabole':
                        import calendar
                        start_date = None

                        todays_date = datetime.now()
                        fin_expiry = (todays_date.replace(day=calendar.monthrange(todays_date.year, todays_date.month)[1]))
                        final_expiry = fin_expiry.replace(hour=23, minute=59, second=00, microsecond=000000)

                        info = "trans_id: %s - msisdn: %s - Updating PARABOLE Offer Expiry to : %s" % (str(trans_id),
                        str(msisdn),str(final_expiry))
                        log(resources, info, 'info')

                        update_parabole_offer(resources, PARABOLE_OFFER, start_date, final_expiry)

                        info = "trans_id: %s - msisdn: %s - Updating PARABOLE DA Expiry to : %s" % (str(trans_id),
                        str(msisdn),str(final_expiry))
                        log(resources, info, 'info')

                        update_parabole_da(resources, PARABOLE_DA, final_expiry)
...
def is_barred_MyMeg5(resources):
        """
        checks whether MyMeg5 subscription is allowed
        """
        ...
        if 'active_offers' or 'inactive_offers' in resources['parameters']:
            if(any(offer.get('offer_id', None) == 1253 for offer in (resources['parameters']['active_offers']))):
                info = "MSISDN: %s MyMeg5 Allowed - 1253 Available in Active_offers" % (resources['parameters']['msisdn'])
                log(resources,info,'info')
                return False
            elif(any(offer.get('offer_id', None) == 1253 for offer in (resources['parameters']['inactive_offers']))):
                info = "MSISDN: %s MyMeg5 Allowed - 1253 Available in Inactive_offers" % (resources['parameters']['msisdn'])
                log(resources,info,'info')
                return False
            else:
                info = "MSISDN: %s MyMeg5 NOT Allowed - No Offer 1253" % (resources['parameters']['msisdn'])
                log(resources,info,'info')
                return True
            
        else:
             info = "MSISDN: %s MyMeg5 NOT Allowed - No Offers" % (resources['parameters']['msisdn'])
             log(resources,info,'info')
             return True
