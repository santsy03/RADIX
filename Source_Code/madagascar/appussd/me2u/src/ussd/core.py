...
class Menu(object):
    ...
    def fetchContent(self, source, resources):
        try:
            ...
            params['recipient'] = parameters['recipient']
            #url = (Template(source)).safe_substitute(resources['parameters'])
            url = source
            proxy = xmlrpclib.ServerProxy(url)
            resp = proxy.handleRequest(params)
            resources['ussdResponseString'] = str(resp)
            resources['action'] = 'end'
            return resources
        except Exception, err:
            xmlrpc_error = 'operation:fetchContent failed. Error: %s' % str(err)
            log(resources, xmlrpc_error, 'error')
            resources['ussdResponseString'] = 'XMLERROR'
            resources['action'] = 'end'
            return resources


    def fetchContent_http(self,source,resources):
        ...
        try:
            ...
            print 'error',str(e)
            resources['ussdResponseString'] = USSD_MESSAGES['error']
            resources['action'] = 'end'
            return resources
    ...
    def provisionService(self,resources):
        ...
        try:
            log(resources, 'Error: check here', 'error')
    ...
    def start(self,resources):
        ...
        if self.leaf and (resources['parameters'])['service'] and (resources['parameters'])['package']:
            ...
            resources['ussdResponseString'] = USSD_MESSAGES['defered_response'] 
            resources['action'] = 'end'
        else:
            if self.service:
