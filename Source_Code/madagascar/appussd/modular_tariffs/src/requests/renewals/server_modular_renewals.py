...
class Consumer(object):
    ...
    def handle_delivery(self,channel,method,header,body):
        try:
            from modular_tariffs.src.requests.renewals.core import process_renewal_request
            from utilities.common.core import log
            ...
            try:
                log({'logger':self.logger}, 
                        'operation:handleDelivery,status:failed to process_renewal_request,command:%s,error:%s' %(
                            message,str(e),), 'error')
            except Exception:
                pass
...
def setup(log):
    ...
    db_string = db['string']
    try:
        cwd = '%s/src/requests/renewals' % HOME
        resources = {}
...
class Dequeue(Daemon):
    ...
    def run(self):
        ...
        workers = THREADS['renewals']
        self.resources = setup('log-modular-requests-renewals')
...
if __name__ == '__main__':
    ...
    from modular_tariffs.src.configs import HOME
    cwd = '%s/src/requests/renewals' % HOME
    daemon = Dequeue('%s/requests_renewals.pid' %(cwd))
    if len(sys.argv) == 2:
        if 'start' == sys.argv[1]:
