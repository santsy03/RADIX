...
class menu0():
    def processAction(self,resources):
        ...
        if resources['parameters']['ussdRequestString'] in ('100','101'):
            # by-pass auto renewal menu
            parameters = resources['parameters']
            if parameters['ussdRequestString'] == '100':
                log( resources, '%s - stop renewal request' % parameters['msisdn'], 'debug' )
            else:
                log( resources, '%s - balance check request' % parameters['msisdn'], 'debug' )

            service_key = str(parameters['service_key']).replace('*', '')
            sequence = resources['parameters']['ussdRequestString']
...
class menuRenewal():
    ...
    def processAction(self,resources):
        parameters = resources['parameters']
        request = (parameters['ussdRequestString']).strip()
        if request == '1':
            renew = True
        elif request == '2':
            renew = False
...
if __name__ == '__main__':
    ...
    resources['parameters'] = {'msisdn':'261336173681','package_id':'0'}
    test = menuProcess()
    test.start(resources)
