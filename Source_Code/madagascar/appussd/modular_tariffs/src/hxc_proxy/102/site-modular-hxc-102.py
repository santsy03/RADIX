...
port = PORTS['hxc_proxy']['102']
threads = THREADS['hxc_proxy']

hxcServer = MenuService()
hxcProxyService = internet.TCPServer(port, server.Site(hxcServer))
hxcProxyService.setName('modular_hxc_proxy_102')
application = service.Application('modular_hxc_proxy_102')
logfile = DailyLogFile("logs/twistd.log-modular-hxcproxy102",".")
