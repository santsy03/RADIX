...
class Consumer(object):
    def __init__(self,id,resources):
        queue_name = QUEUES['renewal_responses']
...
def setup(log):
    ...
    db_string = db['string']
    try:
        cwd = '%s/src/responses/renewals' % HOME
        resources = {}
...
class Dequeue(Daemon):
    ...
    def run(self):
        ...
        workers = THREADS['responses']['renewals']
        self.resources = setup('log-modular-responses-renewals')
...
if __name__ == '__main__':
    ...
    from modular_tariffs.src.configs import HOME
    cwd = '%s/src/responses/renewals' % HOME
    daemon = Dequeue('%s/responses_renewals.pid' %(cwd))
    if len(sys.argv) == 2:
        if 'start' == sys.argv[1]:
