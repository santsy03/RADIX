...
def get_offers(resources):
    ...
    stop = datetime.now()
    duration = stop - start
    log(resources, 'time: get_offers %s' %(str(duration.total_seconds())), 'info')
    return resp
...
def refill(resources):
    ...
    return air.Refill(msisdn, refill_id, amount)
...
def updateUsageThresholdsAndCounters(resources):
    ...
    return air.updateUsageThresholdsAndCounters(msisdn, uc, ut, thresh_value)
...
def getUsageThresholdsAndCounters(resources):
    ...
    parameters = resources['parameters']
    msisdn = parameters['msisdn']
    return air.GetUsageThresholdsAndCounters(msisdn)
