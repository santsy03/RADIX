...
from config import AIR_TIMEOUT
	...
	def make_connection(self, host):
		if self.connection is None:
			...
			if self.scheme == self.HTTP:
				self.connection = httplib.HTTPConnection(host, timeout=AIR_TIMEOUT)
			else:
				if not x509:
					x509 = dict()
				#self.connection = httplib.HTTPSConnection( host, None, **x509 )
				self.connection = httplib.HTTPSConnection( host, timeout=AIR_TIMEOUT, **x509 )
