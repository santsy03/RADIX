...
def get_air(resources):
    ...
    from utilities.data_ucip.config import hosts,load
    import random
    ...
    #host = hosts[0]
    host = air_select(hosts, load)
    #host = hosts[int(params['transactionId'])%2]
    log(resources, 'AIR HOST....'+str(host), 'debug') 
    return (AIRHandler(params, host), host)

def air_select(air_hosts, loads):
    import random
    total_load = float(sum(loads))
    relative_weight = [w / total_load for w in loads]

    # Probability for each element
    probs = [sum(relative_weight[:i + 1]) for i in range(len(relative_weight))]
    for (i, air) in enumerate(air_hosts):
...
def bill_subscriber(resources, transaction_id=''):
    ...
    try:
        ...
        bill_error = 'operation: utilities.ucip.core.bill_subscriber() failed. - %s -%s -packageId:%s- AirHost:%s' % (
                str(err), str(msisdn), str(pack), str(airInstance[1]) )
        log(resources, bill_error, 'error')
        raise err

def bill_subscriber2(resources, transaction_id=''):
    '''
    method for billing a prepaid subscriber
    '''
    airInstance = get_air(resources)
    ...
    try:
        ...
        bill_error = 'operation: utilities.ucip.core.bill_subscriber() failed. - %s -%s - AirHost:%s' % (
...
def set_service_class(resources):
    ...
    try:
        ...
        error = "operation: set_service_class failed. - %s -%s - AirHost:%s" % (
                str(err), str(msisdn), str(airInstance[1]))
...
def get_balance_and_date(resources):
    ...
    try:
        ...
        error = 'operation: get_balance_and_date failed. - %s- %s- AirHost:%s' %(
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
def set_offer_id(resources):
    ...
    try:
        ...
        error = 'operation: set_offer_id. - %s - %s - AirHost:%s' % (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
def set_value_offer_id(resources):
    ...
    try:
        ...
        error = 'operation: set_value_offer_id. - %s - %s - AirHost:%s' % (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
def get_offers(resources):
    ...
    try:
        ...
        error = 'operation:get_offers failed:- %s - %s -packageId:%s- AirHost:%s' % (
                str(err), str(msisdn), str(pack), str(airInstance[1]))
        log(resources, error, 'error')
        raise err

def get_offers2(resources):
    '''
    method for retrieving currently provisioned offers
    '''
    airInstance = get_air(resources)
    ...
    try:
        ...
        error = 'operation:get_offers failed:- %s - %s - AirHost:%s' % (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
def delete_offers(resources):
    ...
    try:
        ...
        error = 'operation:delete_offers - failed: -%s - %s - AirHost:%s'% (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
def update_da_account(resources, data_unit = 6):
    ...
    try:
        ...
        error = 'operation:update_da_account - failed: -%s - %s - AirHost:%s'% (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
def get_sub_balance(resources, dtype = 'A'):
    ...
    try:
        ...
        error = 'operation:get_sub_balance - failed: -%s - %s - AirHost:%s'% (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err 
...
def refill(resources):
    ...
    try:
        ...
        log(resources, 'time: refill: %s - %s -packageId:%s- AirHost:%s' % (duration, str(msisdn), str(pack), airInstance[1]), 'info')
        return resp
    except Exception, err:
        #error = 'operation:refill - failed: -%s - %s - AirHost:%s'% (str(err), str(msisdn), str(airInstance[1]))
        error = 'operation:refill - failed: -%s - %s -packageId:%s - AirHost:%s'% (str(err), str(msisdn), str(pack), str(airInstance[1]))
...
def updateUsageThresholdsAndCounters(resources):
    ...
    try:
        ...
        error = 'operation:updateUsageThresholdsAndCounters - failed: -%s - %s -PackageId:%s- AirHost:%s'% (
                str(err), str(msisdn), str(pack), str(airInstance[1]))
        log(resources, error, 'error')
        raise err

def updateUsageThresholdsAndCounters2(resources):
    '''
    method for updating usage counters and thresholds for data
    '''
    airInstance = get_air(resources)
    ...
    try:
        ...
        error = 'operation:updateUsageThresholdsAndCounters - failed: -%s - %s - AirHost:%s'% (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
def getUsageThresholdsAndCounters(resources):
    ...
    try:
        ...
        error = 'operation:getUsageThresholdsAndCounters - failed: -%s - %s -PackageId:%s- AirHost:%s'% (
                str(err), str(msisdn), str(pack), str(airInstance[1]))
        log(resources, error, 'error')
        raise err

def getUsageThresholdsAndCounters2(resources):
    '''
    method for getting the usage threshold and counters
    '''
    airInstance = get_air(resources)
    ...
    try:
        ...
        error = 'operation:getUsageThresholdsAndCounters - failed: -%s - %s - AirHost:%s'% (
                str(err), str(msisdn), str(airInstance[1]))
        log(resources, error, 'error')
        raise err
...
if __name__ == '__main__':
    ...
    parameters['msisdn'] = '261330494860'
    ...
    parameters['adjustmentAmount']  = '16106127360'
    parameters['dedicatedAccountId']  = 1024
