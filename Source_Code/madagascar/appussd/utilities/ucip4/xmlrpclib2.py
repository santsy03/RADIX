...
# 2010-03-05 Omid Add iso8601 with timezone support for CS3g project
#
...
# Format datetime in iso8601 format. Prior methods ignored timezone
# information (Truncates microseconds, adds TZD, changes +00:00 to Z)
def _iso8601format(dt):
    if dt.tzinfo:
        tmp = dt.replace(microsecond=0).isoformat().replace('-', '')
        tz = tmp[18:].replace(':','')
        tmp = tmp[:18] + tz
        return tmp
#        return dt.tzinfo.normalize(dt.replace(microsecond=0)).isoformat() \
#           .replace('+00:00', 'Z')
    return dt.replace(microsecond=0).isoformat().replace('-', '') + '+0000'
...
#        print '\n', value, '\n'
        if isinstance(value, StringType):
            ...
            self.value = _iso8601format(value)
        else:
