def setup():
    ...
    resources['connection'] = resources['connections']
    return resources
...
def test_getBalance():
    from lib_modular.src.incremental.core import getBalance
    resources = setup()
    parameters = {}
...
def test_provision():
    from lib_modular.src.incremental.core import provisionBundle
...
if  __name__=='__main__':
    ...
    start = dt.now() 
    print 'TIME start - %r' % str(start)
    test_getBalance()
    stop = dt.now()
    mduration = (stop - start).microseconds
    duration = (stop - start).seconds
    print 'TIME stop - %r' % str(stop)
    print 'TIME taken - %r microseconds' % mduration
    print 'TIME taken - %r seconds' % duration
