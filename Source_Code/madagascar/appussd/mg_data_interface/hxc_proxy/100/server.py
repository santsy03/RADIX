...
class MenuService(xmlrpc.XMLRPC):
    def __init__(self):
        ...
        self.connectionPools['core'].timeout = 300
        self.allowNone = 1
        self.useDateTime = 1
        self.response = {}
    ...
    def xmlrpc_USSD_MESSAGE(self,params):
        ...
        if 'USSD_BODY' in params and str(params['SEQUENCE']) == '0':
            ...
            elif len(params['USSD_BODY'].split('*')) == 3:
                msisdn = params['USSD_BODY'].split('*')[2]
                package = params['USSD_BODY'].split('*')[0]
                code = params['USSD_BODY'].split('*')[1]
                parameters['INPUT'] = '%s||%s||%s' %(package, code, msisdn)
    ...
    def processRequest(self,resources):
        ...
        inp = parameters['ussdRequestString']
        ...
        if len(inp.split('||')) == 3:
            if inp.split('||')[1] != '121':
                log(resources, '%s -- WRONG DIAL STRING,%s'%(msisdn,str(inp)))
                return self.debugging('Sorry. You are not allowed to access this service')
